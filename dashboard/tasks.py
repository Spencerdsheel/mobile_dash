...
# from .utils import chunk_and_cache_df
...
# def get_last_created_at_key(table_key):
#     redis_val = redis_client.get(f"{table_key}_latest_created_at")
#     if redis_val:
#         return datetime.strptime(redis_val.decode(), "%Y-%m-%d %H:%M:%S")
#     return datetime(2023, 11, 23, 0, 0, 0)

# def update_created_at_key(table_key, latest_timestamp):
#     redis_client.set(f"{table_key}_latest_created_at", latest_timestamp.strftime("%Y-%m-%d %H:%M:%S"))
...
#         booking_rows = cursor.fetchall()
#         if booking_rows:
#             booking_df = pd.DataFrame(booking_rows, columns=booking_cols)
#             chunk_and_cache_df(booking_df, booking_key)
#             update_created_at_key(booking_key, booking_df["created_at"].max())
#             print(f"✅ Cached {len(booking_df)} new booking rows.")
#         else:
#             print("⚠️ No new booking data.")
...
#         validator_rows = cursor.fetchall()
#         if validator_rows:
#             validator_df = pd.DataFrame(validator_rows, columns=validator_cols)
#             chunk_and_cache_df(validator_df, validator_key)
#             update_created_at_key(validator_key, validator_df["created_at"].max())
#             print(f"✅ Cached {len(validator_df)} new validator rows.")
#         else:
#             print("⚠️ No new validator data.")
...
#             chunk_and_cache_df(user_df, user_key)
#             update_created_at_key(user_key, user_df["created_at"].max())
#             print(f"✅ Cached {len(user_df)} new user rows.")
#         else:
#             print("⚠️ No new user data.")
...
#     except Exception as e:
#         print("🚨 Error in update_all_tables_cache:", e)
#         return str(e)
